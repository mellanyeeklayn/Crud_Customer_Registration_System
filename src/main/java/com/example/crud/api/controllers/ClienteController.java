...
import com.example.crud.api.responses.Response;
...
public class ClienteController {
	...
	@PostMapping
	public ResponseEntity<Response<Cliente>> cadastar(@Valid @RequestBody Cliente cliente, BindingResult result){
		if(result.hasErrors()) {
	...
	@PutMapping(path = "/{id}")
	public ResponseEntity<Response<Cliente>>atualizar(@PathVariable(name = "ïd")String id,@Valid @RequestBody Cliente cliente, BindingResult result){
		if(result.hasErrors()) {
			List<String> erros = new ArrayList<String>();
			result.getAllErrors().forEach(erro-> erros.add(erro.getDefaultMessage()));
			return ResponseEntity.badRequest().body(new Response<Cliente>(erros));
	...
	public ResponseEntity<Response <Integer>>remover(@PathVariable(name = "ïd")String id){
